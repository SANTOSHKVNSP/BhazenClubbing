...
export function SeatSelect({ seating, slug }: { seating: ShowtimeSeating; slug: string }) {
  ...
  const isHybrid = seating.sections.length > 0 && seating.ga.length > 0;
  const [tab, setTab] = useState<"passes" | "seats">("passes");
  ...
  const passesBlock = seating.ga.length > 0 && (
  ...
  const seatsBlock = seating.sections.length > 0 && (
    <>
  ...
  return (
    <div className="pb-28">
      {isHybrid ? (
        <>
          <div className="mx-auto mb-8 flex max-w-xs gap-1 rounded-full bg-white/10 p-1 text-sm font-bold">
            <button type="button" onClick={() => setTab("passes")} className={`flex-1 rounded-full px-4 py-2 transition ${tab === "passes" ? "bg-white text-ink" : "text-white/70 hover:text-white"}`}>Passes</button>
            <button type="button" onClick={() => setTab("seats")} className={`flex-1 rounded-full px-4 py-2 transition ${tab === "seats" ? "bg-white text-ink" : "text-white/70 hover:text-white"}`}>Premium seats</button>
          </div>
          {tab === "passes" ? passesBlock : seatsBlock}
        </>
      ) : (
        <>
          {passesBlock}
      ...
      {/* Footer bar (totals across both tabs) */}
          ...
          <div className="text-sm text-white/80" aria-live="polite">
            {count > 0 ? (
              <>
                ...
                {isHybrid && selectedIds.length > 0 && gaCount > 0 && (
                  <span className="ml-2 text-xs text-white/50">({selectedIds.length} premium + {gaCount} pass)</span>
                )}
              </>
            ) : (
