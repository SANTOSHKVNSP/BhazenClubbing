...
## Changelog
- **2026-07-02** — **SEO fix (indexing)**: `NEXT_PUBLIC_SITE_URL` was still the old `sattvick-beats.vercel.app`, so **robots.txt + sitemap.xml advertised the Vercel URL instead of the custom domain** — Google was pointed at the wrong host. Set the Vercel env var to `https://sattvikbeats.com`, aligned code fallbacks + `metadataBase` to the apex, and added **per-page canonical tags** (home + event, resolved against the apex) so apex/www/vercel duplicates consolidate. Follow-up (user): verify the domain in **Google Search Console** + submit the sitemap + Request Indexing. (New domains still take days–weeks to appear.)
- **2026-07-02** — **P10.1 — UX polish**: header is now a **fixed, scroll-aware nav** (`components/event/site-header.tsx` — transparent over the hero → solid blurred bar on scroll) so the *Reserve Your Spot* button stays visible throughout the page; added `scroll-pt-20` so anchor jumps clear the fixed header. Removed the *Feel the Energy* (experience) section and the *Exclusive Event Merchandise* highlight. Deployed.
- **2026-07-02** — **P10 — BhaZen Jamming 2.0 fundraiser redesign** (content + template): reframed the event page as a **Green Vizag fundraiser**. New hero ("Where Every Beat Plants a Future" · Music•Meditation•Plantation), **trimmed to fit one screen**; new sections — More Than a Concert, What Awaits You, Your Ticket Creates Change, Feel the Energy, Gurudev quote, Every Beat Plants a Future. Band = **The Nirvana Station** with **8 named members** (gradient initials avatars + bio). Time → **5 PM**, venue → **Port Indoor Stadium**. Tiers renamed for clarity: Gallery ₹399 / Arena ₹599 / **Premium (Seat Only)** ₹1,999 / **Premium + Sudarshan Kriya** ₹2,999, plus a dedicated **Sudarshan Kriya / Happiness Program** panel (complimentary 3–4 day course, late July) and a **bulk-passes** panel (4/10-pass bundles booked via the AOL donation link → "Other Amount"). Expanded `EventContent` (headline, highlights, impact, experience, quote, finalCta, admissionLabels, ticketPerks, happinessProgram, bulkPasses) — **no schema migration**. CTAs restored to **Reserve Your Spot**. Deployed to prod + reseeded.
- **2026-07-02** — **Legacy site retired + Vercel deploys cleaned**: `bhazenclubbing.com` (Cloudflare Pages, `main`) now **301-redirects to `sattvikbeats.com/e/bhazen-jamming`** (catch-all `_redirects` + meta-refresh stub). Custom domain **went live** (GoDaddy NS → `vercel-dns`; apex+www serve with SSL). Removed **14** stale/failed Vercel deployments (kept only the aliased live prod). Note: pushing to `main` triggers a failing Vercel git-build (no app on `main`) — recommend an *Ignored Build Step* for `main`.
- **2026-07-01** — **P8 + P9 deployed to production**: `vercel deploy --prod` (ticketing_mode migration applied via `vercel-build`) + prod reseeded to the **single live event** (BhaZen Jamming, external `aolt.in` tiers, indoor copy, @bhazen_jamming). Verified live at `sattvick-beats.vercel.app` (favicon=logo, health ok, external links, demo slugs → 404 page). **Custom domain** `sattvikbeats.com` + `www` added to the project — **awaiting GoDaddy DNS** (A → 76.76.21.21 for apex+www, or NS → vercel-dns). Known follow-ups: **OTP delivery** (stub) + **soft-404 status** (200 body-404 for unknown slugs on Next 16).
